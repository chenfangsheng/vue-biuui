<template>
  <div>
    <biu-mask v-model="show" @click.native="close" ref="mask"></biu-mask>
    <div class="biu-linkage" :class="show ? 'biu-linkage-active' : ''">
      <div class="biu-linkage-header">
        <p class="biu-linkage-title">{{title}}</p>
        <div v-show="ready" class="biu-linkage-nav">
          <a href="javascript:;"
             :key="key"
             v-for="(index, key) in columnNum"
             v-show="!!nav['txt' + index]"
             @click.stop="navEvent(index)"
             :class="index == navIndex ? 'biu-linkage-nav-active' : ''"
          >{{nav['txt' + index]}}</a>
        </div>
      </div>
      <div v-if="!ready" class="biu-linkage-loading">
      ...
      <ul v-show="ready" class="biu-linkage-content" :class="activeClasses">
        <li class="biu-linkage-item" v-for="(index, key) in columnNum" :ref="'itemBox' + index" :key="key">
          <template v-if="columnsObj['columnItems' + index] && columnsObj['columnItems' + index].length > 0">
            <div class="biu-linkage-item-box">
              <a href="javascript:;"
                 :key="key"
          ...
          <template v-else>
            <div class="biu-linkage-item-box" @touchstart.stop.prevent=""></div>
          </template>
        </li>
      </ul>
    </div>
...
export default {
  ...
  methods: {
    ...
    currentClass(v, n, index) {
      return (v && v == this.active['itemValue' + index]) || (n && n === this.active['itemName' + index]) ? 'biu-linkage-item-active' : '';
    },
    clearNavTxt(index) {
    ...
    backoffView(animate) {
      this.activeClasses = (animate ? 'biu-linkage-move-animate' : '') + ' biu-linkage-prev';
    },
    forwardView(animate) {
      this.activeClasses = (animate ? 'biu-linkage-move-animate' : '') + ' biu-linkage-next';
    }
  },
  mounted() {
    this.init();
