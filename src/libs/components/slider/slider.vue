<template>
    <div class="biu-slider" ref="slider">
        <div class="biu-slider-wrapper" ref="warpper"
          :class="direction === 'vertical' ? 'biu-slider-wrapper-vertical' : ''"
          :style="dragStyleObject">
          <div class="biu-slider-item" v-if="loop" :style="itemHeight" v-html="lastItem"></div>
          <slot></slot>
          <div class="biu-slider-item" v-if="loop" :style="itemHeight" v-html="firstItem"></div>
        </div>
        <div class="biu-slider-pagination" v-if="itemsArr.length > 1 && showPagination"
          :class="direction === 'vertical' ? 'biu-slider-pagination-vertical' : ''">
          <span class="biu-slider-pagination-item"
            v-for="t, i in itemNums" :key="i"
            :class="paginationIndex === i ? 'biu-slider-pagination-item-active': ''"
...
export default {
  ...
  methods: {
    init() {
      ...
      this.itemsArr = this.$children.filter(item => item.$options.name === 'biu-slider-item');
