<template>
  <label class="biu-checkbox" :class="shape == 'circle' ? 'biu-checkbox-circle' : ''">
    <template v-if="group">
    ...
    <span class="biu-checkbox-icon" :style="iconStyles()"><i :style="checkIconStyles()"></i></span>

    <template v-if="!$slots.default">
      <span class="biu-checkbox-text">{{val}}</span>
    </template>
    <template v-else>
      <span class="biu-checkbox-text"><slot></slot></span>
    </template>
  </label>
</template>
...
export default {
  ...
  created() {
    if (
      this.$parent.$options &&
      this.$parent.$options.name === "biu-checkbox-group"
    ) {
      this.group = true;
    }
  }
