<template>
  <div>
    ...
    <div class="biu-dateselect" :class="show ? 'biu-dateselect-active' : ''">
      <div class="biu-dateselect-head">
        <a href="javascript:;" @click.stop="close">{{cancelText}}</a>
        <a href="javascript:;" @click.stop="setValue">{{confirmText}}</a>
      </div>
      <div class="biu-dateselect-content">
        <div class="biu-dateselect-item" v-for="(column, key) in columns" :key="key">
          <div class="biu-dateselect-item-box" :ref="'Component_' + column">
            <div class="biu-dateselect-item-content" :ref="'Content_' + column">
        ...
        <div class="biu-dateselect-shade"></div>
        <div class="biu-dateselect-indicator"><span></span></div>
      </div>
    </div>
  </div>
</template>
...
import { isIOS, pageScroll } from "../../../utils";
...
export default {
  ...
  methods: {
    init() {
      ...
      let currentValue = '';
      if(_this.value){
        currentValue = (_this.currentValue = _this.value.replace(/-/g, "/"));
      }else{
        if(_this.currentValue == "") {
          currentValue = _this.currentValue = _this.getNow();
        }else {
      ...
      if (
        _this.startDate &&
        new Date(currentValue).getTime() < new Date(_this.startDate).getTime()
      ) {
      ...
      if (
        _this.endDate &&
        new Date(currentValue).getTime() > new Date(_this.endDate).getTime()
      ) {
    ...
    render() {
      ...
      _this.columns.forEach(item => {
        ...
        _this.scroller[item].setDimensions(
          7 * _this.itemHeight,
    ...
    scrolloToPosition(type, allDatas, callback) {
      ...
      setTimeout(() => {
        ...
        if (!_this.scrolling[type]) {
          scroller.select(
            indatas ? _this["current" + type] : allDatas[0].value,
            false
          );
        }
