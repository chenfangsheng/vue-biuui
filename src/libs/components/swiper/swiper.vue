<template>
  <div class="biu-swiper-wrap">
    <div class="biu-swiper" :style="{height: xheight}">
      <slot></slot>

      <div class="biu-swiper-item" v-for="(item, index) in list" @click="clickListItem(item)" :data-index="index" :key='index'>
        <a href="javascript:">
          <div class="biu-img" :style="{backgroundImage: buildBackgroundUrl(item)}"></div>
          <p class="biu-swiper-desc" v-if="showDescMask">{{ item.title }}</p>
        </a>
      </div>

      <div
      v-if="listTwoLoopItem.length > 0"
      class="biu-swiper-item biu-swiper-item-clone"
    ...
    <div :class="[dotsClass, 'biu-indicator', `biu-indicator-${dotsPosition}`]" v-show="showDots">
      <a href="javascript:;" v-for="(key, index) in length" :key='index'>
        <i class="biu-icon-dot" :class="{'active': key - 1 === current}"></i>
      </a>
    </div>
  </div>
</template>
