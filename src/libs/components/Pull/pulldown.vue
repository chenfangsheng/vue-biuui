...
export default {
  ...
  methods: {
    init() {
      ...
      this.$on("pulldown.finishLoad", this.resetParams);
    },
    bindEvents() {
    ...
    touchEndHandler(event) {
      ...
      this.dragTip.animationTiming = "biu-pullrefresh-animation-timing";

      if (touches.moveOffset >= this.dragTip.distance) {
        this.dragTip.statusText = this.loadingText;
        this.dragTip.loadingIcon = "biu-pullrefresh-loading";
        this.triggerLoad();
        return;
      }
